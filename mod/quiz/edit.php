...
function module_specific_controls($totalnumber, $recurse, $category, $cmid){
    ...
    if (has_capability('moodle/question:useall', $catcontext)){
        ...
        if ($maxrand > 0) {
            ...
            // IP based questions
            $out .= '<br />';
            $out .= get_string('addipbased', 'quiz', choose_from_menu($randomcount, 'randomcount', '1', '', '', '', true));
            $out .= '<input type="hidden" name="recurse" value="'.$recurse.'" />';
            $out .= '<input type="hidden" name="categoryid" value="'.$category->id.'" />';
            $out .= '<input type="input" name="ipbasedeq" value="" />';
            $out .= '<input type="submit" name="addipbased" value="'. get_string('add') .'" />';
            $out .= helpbutton('random', get_string('random', 'quiz'), 'quiz', true, false, '', true);
        }
    }
    return $out;
...
if ((optional_param('addrandom', false, PARAM_BOOL) or
     optional_param('addipbased', false, PARAM_BOOL)) and confirm_sesskey()) { /// Add random questions to the quiz
    $recurse     = optional_param('recurse', 0, PARAM_BOOL);
    $categoryid  = required_param('categoryid', PARAM_INT);
    $randomcount = required_param('randomcount', PARAM_INT);
    $isipbased   = optional_param('addipbased', false, PARAM_BOOL);
    $ipbasedeq   = optional_param('ipbasedeq', false, PARAM_RAW);
    $questiontype = $isipbased ? 'ipbased' : RANDOM;
                ...
                ORDER BY id"))
    {
        ...
        while (($existingquestion = array_shift($existingquestions)) and $randomcount > 0) {
            ...
            if ($isipbased) {
                $q = get_record ('question_ipbased', 'id', $existingquestion->id);
                if ($q) {
                    $q->ipbasedeq = $ipbasedeq;
                    $q->question  = $existingquestion->id;
                    update_record ('question_ipbased', $q);
                } else {
                    $q = new stdClass;
                    $q->question  = $existingquestion->id;
                    $q->ipbasedeq = $ipbasedeq;
                    insert_record ('question_ipbased', $q);
                }
            }
            $randomcount--;
        }
    ...
    if ($randomcount > 0) {
        ...
        for ($i = 0; $i < $randomcount; $i++) {
            ...
            if ($isipbased)
                $question->ipbasedeq = $ipbasedeq;
            $question = $QTYPES[$questiontype]->save_question ($question, $form, $course);
