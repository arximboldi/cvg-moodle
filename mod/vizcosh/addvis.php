...
//the following parameters define the search query a user submitted (input-fields in search-tab (addvis.html))
...
$row[] = new tabobject('list', "addvis.php?tab=list", get_string('listtab', 'vizcosh'), get_string('listtab', 'vizcosh'));
$row[] = new tabobject('search', "addvis.php?tab=search", get_string('searchtab', 'vizcosh'), get_string('searchtab', 'vizcosh'));
$tabs[] = $row;
...
print_header("$course->shortname: $vizcosh->name", $course->fullname, "<a href=\"index.php?id=$course->id\">$strvizcoshs</a> -> <a href=\"view.php?id=$cm->id\">$vizcosh->name</a> -> <a href=\"editparagraph.php?id=$cm->id&chapterid=$chapter->id&paragraphid=$paragraphid&orderposition=$orderposition\">$stredit</a> -> $strlist", '', '', true, '', '');
