...
$formatid = required_param('formatid', PARAM_INT); // algorithm visualization ID (vizalgo selected for updating(deleting or if inserting =-1)
...
if (($form = data_submitted()) && (confirm_sesskey())) {
    ...
    if ((!isset ($form->name) || $form->name == null || !isset ($form->author) || $form->author == null || !isset ($form->template) || $form->template == null) && (!isset ($_REQUEST['load']))) {
        error('Could not add or update your algorithm visualization format. A required field was missing.', $errorurl1);
    }
    //if form was filled correctly: create/edit $vizalgo object to store the user inputs
    //required fields: name, author, date, jnlp_template 
    else {
    ...
    //if user wants to load one of the available jnlp templates into the template field in the form
    if (isset ($_REQUEST['load'])) {
        //load the file from files/visualizers/jnlps and print it to the form
        if (!isset ($jnlp_samples)) {
            $animal_jnlp = fopen("./files/visualizers/jnlps/animal.jnlp", "r");
            $animal_data = fread($animal_jnlp, filesize("./files/visualizers/jnlps/animal.jnlp"));
            $jhave_jnlp = fopen("./files/visualizers/jnlps/jhave.jnlp", "r");
            $jhave_data = fread($jhave_jnlp, filesize("./files/visualizers/jnlps/jhave.jnlp"));
            $jawaa_jnlp = fopen("./files/visualizers/jnlps/jawaa.jnlp", "r");
            $jawaa_data = fread($jawaa_jnlp, filesize("./files/visualizers/jnlps/jawaa.jnlp"));
            $jeliot_jnlp = fopen("./files/visualizers/jnlps/jeliot.jnlp", "r");
            $jeliot_data = fread($jeliot_jnlp, filesize("./files/visualizers/jnlps/jeliot.jnlp"));
            $jnlp_samples = array (
                "animal" => $animal_data,
                "jhave" => $jhave_data,
                "jawaa" => $jawaa_data,
    ...
    //if edit, new or delete 
        ...
        if ($modus == 'edit' && $format) {
            ...
            if (!update_record('vizcosh_vizalgo_formats', $format)) {
                error('Could not update this vizalgo format.', $errorurl2);
            } else {
        ...
        //if user wanted to insert new algorithm visualization 
            ...
            if ($modus == 'new') {
                ...
                if (!$format->id = insert_record('vizcosh_vizalgo_formats', $format)) {
                    print_object($format);
                    error('Could not insert a new algorithm visualization format', $errorurl2);
                } else {
...
if (($modus == 'delete') && ($vizcosh)) {
    ...
    if (!$format->id = delete_records('vizcosh_vizalgo_formats', 'id', $format->id)) {
        error('Could not delete this algorithm visualization format', $errorurl2);
    } else {
        redirect("addformat.php");
    }
    die;
