...
$vizalgoid = required_param('vizalgo', PARAM_INT); // algorithm visualization ID (vizalgo selected for updating(deleting or if inserting =-1)
$modus = required_param('modus', PARAM_TEXT); // new or edit or delete
// =========================================================================
...
// Process user inputs =====================================================
    ...
    if (!isset ($form->title) || $form->title == null || !isset ($form->description) || $form->description == null || !isset ($form->author) || $form->author == null || !isset ($form->format) || $form->format == null) {
        error('Could not add or update your algorithm visualization. A required field was missing.', $errorurl1);
    }
    ...
    if((isset($form->deletethumb))&&($form->deletethumb=="thumbnail")){
      if (!empty ($_FILES) && !empty ($_FILES['thumbnail']['tmp_name'])) {
        $thumb_um = new upload_manager("thumbnail", false, false, null, false, 0, true, true, false);
        if ($thumb_um->preprocess_files()) {
            move_uploaded_file($_FILES['thumbnail']['tmp_name'], "./files/tmp/" . $_FILES['thumbnail']['name']);
    ...
    if(((isset($form->deletethumb))&&($form->deletethumb=="old"))||(!(isset($form->deletethumb)))){
    ...
    //check whether a newl uploaded data file has the correct file extension; it must match extension the chosen format defines 
    $vizalgoformat = get_record('vizcosh_vizalgo_formats', 'name', $vizalgo->format);
    if (isset ($vizalgo->fndata) && $vizalgo->fndata != null && $vizalgo->fndata != "" && isset ($vizalgo->data) && $vizalgo->data != null && $vizalgo->data != "") {
        if (isset ($vizalgoformat->extension) && $vizalgoformat->extension != null && $vizalgoformat->extension != "") {
            if (!preg_match("/\." . $vizalgoformat->extension . "$/i", $vizalgo->fndata)) {
                error("Could not update your algorithm visualization. You need to specify a data file matching the chosen format. The following filetype is required: " . $vizalgoformat->name . " *." . $vizalgoformat->extension, $errorurl1);
            }
        }
    }
...
// create jnlp-file used to start algorithm visualization============================
    ...
    //replace the variables in jnlp_template of the chosen format with the appropriate values
    $jnlp_text = $vizalgoformat->jnlp_template;
    //jnlp-file will be stored in the temporary directory of the user who starts the visualization, so
    //this variable ist just replaced by another variable which will be replace just before starting the jnlp
    $jnlp_text = str_replace("<JNLP-PATH>", "<TMP-PATH>", $jnlp_text);
    //this defines the path to the jars used (animal, jhave and jawaa)
    $jnlp_text = str_replace("<JAR-PATH>", $CFG->wwwroot . "/mod/vizcosh/files/visualizers/jars", $jnlp_text);
    //if algorithm visualization also needs a data file, this will also be stored in the temporary directory of the user
    if (strpos($jnlp_text, "<DATA-PATHFILENAME>") != false) {
        if (isset ($vizalgo->fndata) && $vizalgo->fndata != null && $vizalgo->fndata != "") {
            $jnlp_text = str_replace("<DATA-PATHFILENAME>", "<TMP-PATH>" . $vizalgo->fndata, $jnlp_text);
        } else {
            error("Could not update your VizCoSH. You need to specify a data file for this format.", $errorurl1);
        }
    }
