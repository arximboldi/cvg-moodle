<?PHP // $Id: backuplib.php,v 1.1 2006/03/12 18:39:59 skodak Exp $
    ...
    function vizcosh_backup_mods($bf,$preferences) {
        ...
        if ($vizcoshs = get_records ('vizcosh', 'course', $preferences->backup_course, 'id')) {
    ...
    function backup_vizcosh_paragraphs($bf,$preferences,$vizcosh, $ch) {
        ...
        //Print vizcosh's chapters
		if ($paragraphs = get_records_sql('SELECT * FROM '.$CFG->prefix.'vizcosh_paragraphs WHERE vizcoshid = '.$vizcosh->id.' AND chapterid = '.$ch->id.' ORDER BY orderposition')) {		
            //Write start tag
