	...
	if (!$all_paragraphs_array = get_recordset_select('vizcosh_paragraphs', 'vizcoshid = '.$vizcosh->id.' AND chapterid = '.$chapterid, 'orderposition')){
		...
		if ($all_paragraphs_array->_numOfRows > 0){
			...
			if (isset($USER->editing) && ($USER->editing==1)){
					$markedContent .= '<br><br><a title="'.get_string('edit').'" href="editparagraph.php?id='.$cm->id.'&amp;chapterid='.$chapterid.'&paragraphid=-1&orderposition=1"><img src="pix/add.gif" height="11" class="iconsmall" alt="'.get_string('edit').'" />Absatz hinzuf&uuml;gen</a>';
			}
			else{
				$markedContent .= 'Um Inhalt hinzuzuf&uuml;gen, wechseln Sie in den Bearbeiten-Modus.</div>';
	...
	if (!$all_paragraphs_array = get_recordset_select('vizcosh_paragraphs', 'vizcoshid = '.$vizcosh->id.' AND chapterid = '.$chapterid, 'orderposition')){
		...
		if ($all_paragraphs_array->_numOfRows > 0){
			...
			if (isset($USER->editing) && ($USER->editing==1)){
				...
				$Content .= 'Um Inhalt hinzuzuf&uuml;gen, wechseln Sie in den Bearbeiten-Modus.</div>';
			}
		}
	}
